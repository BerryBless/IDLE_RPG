...
## 알려진 제약 (Task 2 라이브 검증에서 발견, 2026-07-04)

`git checkout claude`는 워킹트리에 미스테이지 변경사항이 있고 그 변경사항이 `claude` 브랜치에 이미
커밋된 같은 파일의 내용과 충돌하면 실패한다(git의 표준 동작: "로컬 변경사항이 checkout으로 덮어써짐").
이 경우 스크립트는 설계대로 폴백해 원래 브랜치(master/main)에 그대로 커밋한다 — 데이터 손실은 없지만
"master를 오염시키지 않는다"는 목표는 이 특정 케이스에서 지켜지지 못한다.

**실사용 발생 조건:** "전환 후 claude에 계속 머무름" 설계 덕분에 연속 세션에서는 거의 발생하지
않는다(한 번 전환되면 계속 claude에 있으므로). 발생하려면 사용자가 세션 사이에 수동으로 master로
되돌아가 claude 브랜치에 이미 있는 것과 다른 내용으로 같은 파일을 편집해야 한다 — 드문 edge case.

**결정: 현재 폴백 동작을 최종 사양으로 채택한다** (git stash 기반 충돌 회피 로직은 추가하지 않음).
이유: stash push → checkout → stash pop 방식은 stash pop 자체도 충돌할 수 있어 무인 자동 훅에
새로운 실패 모드를 추가한다. 반면 현재 폴백은 이미 데이터 손실 없이 안전하게 동작하며, 발생 빈도가
낮은 edge case에 복잡도를 추가하는 것보다 "항상 완주한다"는 자동 훅의 핵심 원칙을 지키는 편이 낫다.
이 케이스가 실사용에서 자주 발생하면(예: master에 남은 커밋이 반복적으로 쌓이면) 재검토한다.
