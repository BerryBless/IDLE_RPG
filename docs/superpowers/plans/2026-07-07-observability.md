...
### Task 3: BattleLoop 콘솔 제거 + BattleEventLogger 삭제

**Files:**
- Modify: `GameServer/Systems/BattleLoop.cs`
- Delete: `GameServer/Systems/BattleEventLogger.cs`
...
- [ ] **Step 2: `BattleEventLoggerTests.cs` 삭제**

```bash
rm tests/GameServer.Tests/Systems/BattleEventLoggerTests.cs
```

- [ ] **Step 3: `BattleLoop.cs` 수정 — `LogTick`을 `GameEventSink` 기반으로 교체**
...
- [ ] **Step 4: `BattleEventLogger.cs` 삭제**
...
Expected: 0 error, 0 warning (기존 `BattleEventLogger` 참조가 모두 제거됐는지 컴파일러가 확인해줌)
...
git rm tests/GameServer.Tests/Systems/BattleEventLoggerTests.cs GameServer/Systems/BattleEventLogger.cs
git commit -m "$(cat <<'EOF'
...
포맷팅이 유일한 역할이었던 BattleEventLogger는 삭제(BattleTickEvent
enum은 이미 BattleLoop.cs에 있어 이동 불필요).
...
Run: `dotnet test tests/GameServer.Tests/GameServer.Tests.csproj`
Expected: 118개 전부 PASS(`Main.cs`는 테스트 대상이 아니므로 영향 없음)

- [ ] **Step 5: 수동 실행으로 콘솔 무출력 + 파일/메트릭 확인**
...
아무 것도 출력하지 않는다. logs/를 .gitignore에 추가.
