...
**현재 상태:** `ServerLib`(고성능 소켓 서버 라이브러리) 기반 위에 `GameServer`(레이드 co-op
전투)·`AuthServer`(로그인)·`MonitorServer`(웹 대시보드) 3개 실행형 서버와 `examples/`의
Echo 사용 예제, `tests/`의 테스트 스위트가 구성되어 있다. 캐릭터 성장·경제 시스템 등
나머지 게임 도메인은 아직 설계되지 않았다. 설계가 확정되는 대로 이 섹션과 `plan/` 문서를
갱신할 것.

**전체 스택 통합 테스트(`tests/FullStack.Tests`):** AuthServer(로그인)+GameServer(HMAC 토큰 게이트+
텔레메트리)+MonitorServer(텔레메트리 구독)를 한 프로세스·루프백에 조합해 `login→token→game-auth→
telemetry→monitor` 전 경로를 검증하는 유일한 4서버 통합 테스트. AuthServer는 인메모리 계정(Mongo
불필요), MonitorServer는 Kestrel 없이 `TelemetryClientLoop`→`TelemetrySnapshotStore`만 구동. 4케이스:
정상 전경로·잘못된 비밀번호 거부·위조 토큰 거부·다중 클라 모니터 반영.
...
- `WebClient`: 브라우저에서 직접 플레이 가능한 게스트 플레이 게이트웨이(`Program.cs`, ASP.NET Core,
  ServerLib만 참조). `GET /`(포트 8081)에서 닉네임 입력→게스트 입장 단일 페이지를 서빙하고, 브라우저당
  WebSocket 1개(`/ws`)를 GameServer(7777) TCP 1개로 브리지(`GameBridge`)한다. 게스트 토큰은 AuthServer/
  Mongo 없이 이 프로세스가 `HmacAuthTokenCodec`으로 직접 발급(`GuestTokenIssuer`, accountId 1_000_000+
  대역) — GameServer와 HMAC 시크릿만 공유하면 된다. 전투는 서버 자동 진행이므로 웹은 보스 HP바·처치/
  MVP 피드를 JSON(`BridgeMessages`)으로 수신만 한다. MVP는 `player-{accountId}-{sessionId}` instanceId를
  `GuestDirectory`로 닉네임 역매핑(SessionAuthGate 포맷 의존). 설계 `plan/web_client_0723.md`.
  실행: `dotnet run --project WebClient` (GameServer 먼저 기동, 포트는 `IDLERPG_WEBCLIENT_WEB_PORT`).
- `tools/LoadTester`: 성능 측정·부하 테스트 콘솔 툴(`Program.cs`, ServerLib만 참조 — 외부 의존성 0).
  가상 클라이언트 N개(10,000+ 지원, 시딩 3000계정 모듈러 재사용)가 `--mode game`(HMAC 토큰 직접
  발급, Mongo 불필요) 또는 `--mode full`(AuthServer 로그인 경유)로 GameServer를 부하하며, 내장
  PING/PONG `Rtt` 샘플링(고정 793버킷 히스토그램 백분위)·응답 정합성(ACK/브로드캐스트 스톨)·연결
  유지율·서버 리소스(WS/CPU, `--server-process`)를 측정해 콘솔 1줄 요약 + `logs/loadtest-*.ndjson`
  (크기·정시 로테이션)으로 기록하고 5규칙 PASS/FAIL 판정(종료코드 0/1/2/3)을 내린다. 72시간 소크
  지원(메모리 상수 상한·Ctrl+C 드레인·텔레메트리 7779 교차 검증). 실행:
  `dotnet run --project tools/LoadTester -- --mode game --clients 100 --duration 60s`
  또는 **`loadtest.bat [clients] [duration] [report-interval]`** 더블클릭 원클릭 실행(Release 빌드
  → GameServer 별도 창 자동 기동 → 테스트 → 서버 정리, 기본 10000/10m/15s. 시크릿은 env >
  .env > 로컬 전용 폴백 순으로 해석). 설계 `plan/loadtester_0721.md`.
  **대규모 동시 연결 용량 하네스(멀티프로세스·멀티포트, `plan/capacity_harness_0721.md`):** 코디네이터가
  워커 K개 프로세스를 스폰해 클라이언트를 샤딩하고, GameServer는 `IDLERPG_GAME_CAPACITY_MODE=1`
  (레이드 브로드캐스트 OFF)+`IDLERPG_GAME_PORT_COUNT=P`(P개 리스너 포트)로 순수 연결 용량을 받는다.
  핵심은 클라이언트가 소스 포트를 명시 바인드(`IClientConnection.LocalEndPoint`+SO_REUSEADDR)해 같은
  소스 포트를 P개 목적지 포트에 재사용 → 단일 소스 IP 임시 포트 상한(~14k)을 우회, **동시 연결 ≈
  소스포트수 × P**로 확장(관리자 권한 불필요). 단일 64GB 머신 동시 3M은 물리적 불가(로컬 루프백
  연결당 소켓 2개+메모리)이나, `capacity-test.bat [clients] [workers] [ports] [duration]`(기본
  300000/8/12/10m)로 수십만 규모를 검증한다. 관리자 `capacity-tune.bat`로 임시 포트 범위를 확장하면
  헤드룸이 더 늘어난다.
  **스트레스 테스트(`plan/stress_harness_0721.md`):** 정상 범위를 넘겨 서버를 깨뜨리고 생존·회복을 보는
  4 시나리오(스파이크/버스트·연결 churn·비정상/악성 패킷·slowloris)를 **측정 전용**(서버 무변경)으로
  구현. 각 실행에 정상 대조군 프로브를 두어 스트레스 전/중/후 서비스 유지·회복을 판정
  (`StressRunner` 페이즈 머신 Baseline→During→Release→Recovery). 실측 대표 발견: 악성 패킷은 서버를
  크래시시키지 못하나 오버사이즈 length 헤더 플러드가 파이프 버퍼를 피어당 수 MB로 증폭(WS 39MB→3.2GB)
  하고 legit RTT를 초 단위로 저하시킴, 정체 세션은 idle sweep 미배선이라 능동 축출 안 됨. 실행:
  `stress-test.bat [scenario] [probe] [stress] [duration]` 또는
  `dotnet run --project tools/LoadTester -- --stress <burst|churn|malformed|slowloris>`.
  **하드닝(opt-in env 토글, 기본 미설정=기존 동작):** `IDLERPG_GAME_IDLE_TIMEOUT_SECONDS=N`(진행 기준
  유휴 스윕 — PING하는 정상 클라는 안전, slowloris·미완성 플러드는 축출, Main.cs 배선), `IDLERPG_GAME_
  MAX_CONNECTIONS=N`(연결 상한, Main.cs 배선), `IDLERPG_GAME_MAX_FRAMES_PER_SECOND=N`(세션당 프레임 레이트
  리밋 — ServerLib 신규 `IServerListener.SessionMaxFramesPerSecond`, 단일 연결 프레임 플러드 완전 차단).
  Before/After 실측: slowloris는 IdleTimeout으로 **완전 방어**(서버 접속 3050→50), 단일 연결 프레임
  플러드는 레이트 리밋으로 완전 차단(E2E 검증), malformed 분산 재접속 플러드는 3종 계층으로 메모리
  **3GB→731MB·연결 850→150·RTT 3.1s→1.4s** + 정상 클라 100% 서비스 유지(잔여 부하는 재접속 폭풍 —
  완전 무영향엔 소스 IP별 연결 레이트 리밋 등 네트워크 경계 방어 필요).

**Docker 컨테이너 스택(신규):** GameServer·AuthServer·MonitorServer·WebClient·MongoDB 5서비스를 한 번에
올릴 수 있다(`docker-compose.yml`, 단일 파라미터화 `Dockerfile`, 설계 `plan/docker_compose_0719.md`).
`copy .env.example .env` 후 `IDLERPG_AUTH_HMAC_SECRET`을 32바이트 이상 값으로 채운 뒤 `up.bat`
더블클릭 한 번으로 빌드+기동, `down.bat`으로 정지(`mongo-data` 볼륨 유지), `seed.bat`으로 더미
계정 3000개 시딩, `logs.bat`으로 로그 추적. 모든 서버가 loopback 바인드/포트를 환경변수로
재정의 가능하게 바뀌었지만 기본값은 기존 로컬 실행과 동일(`dotnet run`은 변경 없이 그대로 동작).
telemetry(7779)·mongo(27017)는 컨테이너 내부망에만 열려 있고 호스트에 publish되지 않는다.

**로컬 실행(Docker 없이):** `run-local.bat`을 더블클릭하면 AuthServer·GameServer·MonitorServer·
WebClient 4개를 각각 별도 콘솔 창에서 `dotnet run`으로 띄운다(Debug 빌드라 `IDLERPG_AUTH_HMAC_SECRET` 없이도
두 서버가 동일한 개발용 기본 비밀키로 자동 합의). 단, AuthServer가 접속할 로컬 MongoDB(mongod,
`mongodb://localhost:27017`)는 이 스크립트가 띄우지 않으므로 실행 전 별도로 켜져 있어야 한다.
...
| [stress_harness_0721.md](plan/stress_harness_0721.md) | 스트레스 테스트 하네스(4 시나리오, 측정 전용): 스파이크/버스트·연결 churn·비정상/악성 패킷·slowloris를 정상 대조군 프로브와 함께 Baseline→During→Release→Recovery 페이즈로 구동해 서버 생존·회복을 판정. 실측: 악성 패킷은 서버 크래시 못 시키나 오버사이즈 헤더 플러드가 파이프 버퍼를 피어당 수 MB 증폭(WS 39MB→3.2GB)+legit RTT 초 단위 저하(FAIL), slowloris/burst/churn PASS. StressRunner/StressVerdictEvaluator/MalformedFrames·AdversarialClient·StalledPeer·WorkerFleet 신규, VirtualClient `--churn`·IClientConnection 무변경. 신규 테스트 25건(152 총). 발견된 약점은 향후 하드닝(Main.cs IdleTimeout/MaxConnections 배선)으로 문서화 |
| [capacity_harness_0721.md](plan/capacity_harness_0721.md) | 대규모 동시 연결 용량 하네스(멀티프로세스·멀티포트): 단일 64GB 머신 동시 3M은 물리적 불가(루프백 연결당 소켓 2개·메모리·O(N) 브로드캐스트)임을 실측 확인. 서버 린 용량 모드(`IDLERPG_GAME_CAPACITY_MODE`, 레이드 미배선)+P리스너 포트, 코디네이터/워커 멀티프로세스, 그리고 **핵심 발견**: 기본 connect()는 소스 포트를 재사용 안 해 단일 소스 IP ~14k에서 정체하나, 소스 포트 명시 바인드+SO_REUSEADDR로 P개 목적지에 재사용하면 동시 연결 ≈ 소스포트수×P로 확장(관리자 불필요, 프로세스 간 재사용까지 실측). 50k/8포트 100% 유지 실증, 300k/12포트 실행. `IClientConnection.LocalEndPoint` 신규(ServerLib), CapacityVerdictEvaluator 5규칙, capacity-tune/test.bat. 신규 테스트 40+건 |
| [loadtester_0721.md](plan/loadtester_0721.md) | 성능 측정·부하 테스트 툴 `tools/LoadTester` 신규: 커스텀 바이너리 프로토콜이라 범용 도구 불가 → ServerLib 클라이언트(내장 PING/PONG RTT) 재활용 전용 콘솔 툴. 가상 클라이언트 10k+(계정 3000 모듈러 재사용), game/full 2모드, RTT 백분위(고정 793버킷)·스톨·유지율·서버 리소스 4계열 측정, 5규칙 PASS/FAIL 판정, NDJSON 로테이션, 72h 소크 대응(락 0·메모리 상수 상한). 신규 테스트 84건 + 실서버 스모크 3종(50/500 클라이언트 PASS·서버 강제종료 FAIL 분류) 실측 검증, 서버 코드 무변경 |
| [web_client_0723.md](plan/web_client_0723.md) | 브라우저 게스트 플레이 게이트웨이 `WebClient` 신규: WebSocket(JSON)↔GameServer TCP(바이너리) 브리지 + 게스트 HMAC 토큰 직접 발급(AuthServer/Mongo 불필요, accountId 1M+ 대역) + 보스 HP/처치·MVP 실시간 표시(닉네임 역매핑 — SessionAuthGate instanceId 포맷 의존). GameServer/ServerLib 무변경, 신규 테스트 39건(단위+실소켓 통합 8건). 능동 입력·실계정 로그인·수평 확장은 다음 사이클 |
| [docker_compose_0719.md](plan/docker_compose_0719.md) | Docker Compose 컨테이너화: GameServer·AuthServer·MonitorServer·MongoDB 4서비스를 `docker-compose.yml`로 묶고, 단일 파라미터화 `Dockerfile`(`ARG PROJECT`/`RUNTIME_IMAGE`)로 3서버가 이미지 정의를 공유. 기존 loopback 하드코딩 바인드/호스트/포트를 환경변수로 구성화하되 기본값은 loopback 유지(로컬 실행 보안 보존), Docker에서만 `0.0.0.0`으로 override. telemetry(7779)·mongo(27017)는 의도적으로 호스트에 미publish. `docker compose down`의 SIGTERM을 잡지 못해 10초 뒤 강제 종료되던 문제를 `PosixSignalRegistration`으로 수정. `up.bat`/`down.bat`/`logs.bat`/`seed.bat`로 Windows 원클릭 운영(.bat는 반드시 ASCII만 사용 — 한글 넣으면 cmd 기본 코드페이지에서 mojibake로 명령 파싱이 깨짐, §7-2 참고). 재시작 정책은 `unless-stopped`→`always`→최종 `restart: "no"`로 결정(§7-1) — 크래시가 나면 자동 재시도하지 않고 `Exited`로 드러나게 두고 수동으로 `up.bat` 재기동. 4컨테이너 기동·텔레메트리 링크·포트 노출 통제·시딩 멱등성·SIGTERM 즉시 종료(1.3초)·볼륨 영속성·크래시 시 미재시작까지 전부 실측 검증 |
